...
describe WorksReport do
  ...
  describe '#create_report' do
    let(:fake_articles) { 
      [ 
        FakeWork.new('pid:1234', 'Title', 'foo@bar.org', 'foo@bar.org', ['pid', 'pid'], ['pid'], ['foo@bar.org']),
        FakeWork.new('pid:1234', 'Title', 'foo@bar.org', 'foo@bar.org', ['pid'], ['pid'], ['foo@bar.org'])
      ]
    }
    ...
    let(:fake_images) { 
      [ 
        FakeWork.new('pid:1234', 'Title', 'foo@bar.org', 'foo@bar.org', ['pid', 'pid'], ['pid'], ['foo@bar.org']),
        FakeWork.new('pid:1234', 'Title', 'foo@bar.org', 'foo@bar.org', [''], ['pid'], ['foo@bar.org']) 
      ] 
    }
    let(:fake_datasets) { 
      [ 
        FakeWork.new('pid:1234', 'Title', 'foo@bar.org', 'foo@bar.org', ['pid', 'pid'], [''], ['foo@bar.org', 'foo@bar.org'])
      ]
    }
    let(:fake_documents) { [] }
    ...
    class FakeWork < Struct.new(:pid, :title, :owner, :depositor, :editor_ids, :editor_group_ids, :edit_users)
    end
  end
end
